...
async function setupTrash() {
    ...
    const stringfiedAmount = trashFormatter.format(amountLeftToRaise)
    const trashAmount = {
        xxl: {
            amount: parseInt(`${stringfiedAmount[0]}${stringfiedAmount[1]}`),
            icon: "bag"
        },
        xl: {
            amount: parseInt(`${stringfiedAmount[2]}`),
            icon: "takeout"
        },
        lg: {
            amount: parseInt(`${stringfiedAmount[3]}`),
            icon: "headphones"
        },
        md: {
            amount: parseInt(`${stringfiedAmount[4]}`),
            icon: "phone"
        },
        sm: {
            amount: parseInt(`${stringfiedAmount[5]}`),
            icon: "toy"
        },
        xs: {
            amount: parseInt(`${stringfiedAmount[6]}`),
            icon: "bottle"
        },
    }
